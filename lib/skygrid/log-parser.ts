...
import type { FlightLog, FlightLogPoint, GeoPoint, LogMetrics, MissionState } from './types';
...
export function parseFlightCsv(text: string, fileName: string, logIndex = 0): FlightLog {
  const lines = text.replace(/^\uFEFF/, '').split(/\r?\n/).filter((line) => line.trim());
  if (lines.length < 3) throw new Error('CSV에 분석 가능한 데이터 행이 없습니다.');
  const delimiter = (lines[0].match(/;/g)?.length ?? 0) > (lines[0].match(/,/g)?.length ?? 0) ? ';' : ',';
  const headers = splitCsvLine(lines[0], delimiter);
  const indexes = {
    timeMs: findIndex(headers, ['timemillisecond', 'timems', 'elapsedms', 'timestampms']),
    ...
    altitude: findIndex(headers, ['heightabovetakeoff', 'altitudeabovesealevel', 'altitude', 'height', 'relativealtitude']),
    speed: findIndex(headers, ['speed', 'horizontalspeed', 'groundspeed', 'velocity']),
    ...
    satellites: findIndex(headers, ['satellites', 'satellitecount', 'gpssatellites']),
    heading: findIndex(headers, ['compassheadingdegrees', 'heading', 'yaw', 'gimbalheadingdegrees']),
    mode: findIndex(headers, ['flycstate', 'flightmode', 'mode']),
  };
  if (indexes.lat < 0 || indexes.lng < 0) throw new Error('위도·경도 열을 찾지 못했습니다. AirData CSV 형식을 권장합니다.');
  ...
  for (const row of rows) {
    ...
    let timeMs = indexes.timeMs >= 0 ? numberAt(row, indexes.timeMs) : numberAt(row, indexes.timeSec) * 1000;
    ...
    points.push({
      ...
      flightMode: indexes.mode >= 0 ? row[indexes.mode] || 'UNKNOWN' : 'UNKNOWN',
    });
  }
  ...
  points.forEach((point, index) => {
    ...
    if (!Number.isFinite(point.speedMps)) {
      ...
      point.speedMps = index === 0 ? 0 : haversineMeters(previous, point) / seconds;
    }
  });
...
export function calculateLogMetrics(log: FlightLog, plannedPath: GeoPoint[] = []): LogMetrics {
  ...
  const speedValues = log.points.map((point) => point.speedMps).filter(Number.isFinite);
  let routeErrorM: number | null = null;
  if (plannedPath.length >= 2) {
    const sampled = log.points.filter((_, index) => index % Math.max(1, Math.floor(log.points.length / 400)) === 0);
    routeErrorM = sampled.reduce((sum, point) => {
      let minimum = Infinity;
      for (let i = 1; i < plannedPath.length; i += 1) minimum = Math.min(minimum, pointToSegmentMeters(point, plannedPath[i - 1], plannedPath[i]));
      return sum + minimum;
  ...
  return {
    durationSec,
    distanceKm,
    avgSpeedMps: speedValues.reduce((sum, speed) => sum + speed, 0) / Math.max(1, speedValues.length),
...
export function createDemoLogs(mission: MissionState): FlightLog[] {
  const generated = mission.drones.slice(0, 2).map((drone, droneIndex) => {
    const targets = drone.route.map((id) => mission.waypoints.find((waypoint) => waypoint.id === id)).filter(Boolean) as GeoPoint[];
    const path = [drone, ...targets];
    const points: FlightLogPoint[] = [];
    let elapsed = 0;
    let battery = drone.battery;
    const endSegment = droneIndex === 1 ? Math.max(1, Math.ceil((path.length - 1) * 0.58)) : path.length - 1;
  ...
  if (generated.length >= 2 && generated[0].points.length && generated[1].points.length) {
    const survivor = generated[0];
    const failed = generated[1];
    const failedEnd = failed.points.at(-1)!;
    const failureTime = failedEnd.timeMs;
    const handoverStartIndex = survivor.points.findIndex((point) => point.timeMs >= failureTime);
    if (handoverStartIndex > 0) {
      ...
      for (let sample = 1; sample <= sampleCount; sample += 1) {
        ...
        retained.push({
          ...
          altitudeM: 42,
          speedMps: 10,
          batteryPercent: start.batteryPercent - fraction * (distance / 1000) * 9.6,
        });
      }
...
export function estimateHandoverSeconds(logs: FlightLog[]): number | null {
  ...
  const sorted = [...logs].sort((a, b) => a.points.at(-1)!.timeMs - b.points.at(-1)!.timeMs);
  ...
  const recovery = survivor.points.find((point) => point.timeMs >= failureTime && haversineMeters(point, failurePoint) <= 120);
